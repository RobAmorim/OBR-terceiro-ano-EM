...
while(millis() < time)
{
  ...
  while(getYPR(0) <= (gyroInicial + 2)&&(getYPR(0) >= (gyroInicial - 2)))
  {  
    
   if((ypr[1] * 180 / M_PI) < -12)
   {
    if((ypr[1] * 180 / M_PI) < -12)
   {
    if((ypr[1] * 180 / M_PI) < -12)
  ...
  { 
    ...
    CorrectSpeed = speedBase + output; 

    if(CorrectSpeed > 255)
    CorrectSpeed = 255; 
  
  
    Robot.speed(CorrectSpeed, CorrectSpeed);
    Robot.right(); 
