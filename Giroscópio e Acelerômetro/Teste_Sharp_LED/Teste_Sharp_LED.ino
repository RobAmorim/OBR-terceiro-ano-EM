...
#define sensorSharpDir A14
#define sensorSharpEsq A15
...
void loop() {
  ...
  //LeituraSharps();
   ...
   int valorSharpDir = analogRead(sensorSharpDir); 
   int valorSharpEsq = analogRead(sensorSharpEsq);
  
  if(valorSharpDir > 500) {
    while(TotalDegrees > -12020*2) {  // -12020*2
      AGRead();
  ...
  if(valorSharpEsq > 500){
    while(TotalDegrees < 11920*2) {  //11920*2
      AGRead();
      TotalDegrees += GDegreesZ;    // 
      //esquerda();
